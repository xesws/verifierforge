...
def _services(cluster_id: str) -> AgentServices:
    ...
    db_path = Path(os.environ.get("VF_PROXY_DB_PATH", str(DEFAULT_DB_PATH))).expanduser()
...
def latest_cluster_decision(cluster_id: str) -> AgentAnalysisResponse:
    _require_enabled()
    try:
        summary = _services(cluster_id).decisions.latest_for_cluster(cluster_id)
...
def approve_decision(decision_id: str, request: ApprovalRequest) -> ApprovalRecord:
    _require_enabled()
    try:
        services = _services("approval-read")
        summary = services.decisions.get(decision_id)
        if summary is None:
            raise HTTPException(status_code=404, detail="Agent decision not found")
        if (
        ...
        return services.approvals.put(decision_id, request.approved_by)
